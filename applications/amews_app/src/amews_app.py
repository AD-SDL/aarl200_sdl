...
if __name__ == "__main__":
    ...
    with experiment_app.manage_experiment(
        ...
        while len(sampled_racks) < total_racks:#len(measured_racks) < total_racks:
            ...
            if bk_workflow and bk_workflow.status.completed:
                ...
                local_protocol_file_path = experiment_app.output_path / "protocols" / f"tube_rack_{len(sampled_racks) + 1}_stamped.json"
                completed_protocol = experiment_app.data_client.save_datapoint_value(
                bk_workflow.get_datapoint_id_by_label("protocol"), local_protocol_file_path
                )
                try: 
                ...
                with open(local_protocol_file_path, "r") as f:
                ...
                try:
                    ...
                    dataset_name = stamp + "_r" + str(len(sampled_racks))
                    sample_info_file_path = create_sample_file(sampled_racks[-1], 
                                                            len(sampled_racks), 
                                                                experiment_app.output_path, 
                                                                "mina_hts_3", 
                                                                dataset_name, 
                                                                post_rinse=True, 
                                                                calibrate=True)
                    try:
                        filename = Path(sample_info_file_path).name
                        with open(experiment_app.network_output_path / filename, "w") as f:
                            f.write(open(sample_info_file_path, "r").read())
                    except Exception as e:
                        print("unable to write sample info file to network")
                except Exception as e:
                    print("unable to create sample info file")
